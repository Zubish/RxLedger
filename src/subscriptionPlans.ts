...
export const trialPolicy = {
  ...
  summary:
    "The free trial includes Smart Pharmacy features for 30 days. Trial data is preserved and can move into any paid plan.",
};

export const planChangePolicy = {
  upgrade: "Upgrades are immediate and keep the same workspace data.",
  downgrade:
    "Downgrades are allowed only when active branches and staff fit the lower plan. Extra data is not deleted; it must be archived, exported, or moved before the lower plan becomes active.",
  dataRetention:
    "Plan changes never delete stock, sales, patient, audit, or branch history.",
};
...
export const subscriptionPlans: SubscriptionPlan[] = [
  {
    ...
    per: "/mo",
    summary:
      "For one independent pharmacy that needs clean stock control and counter sales.",
  ...
  },
  {
    ...
    per: "/mo",
    summary:
      "For growing pharmacies that need patient follow-up and stronger daily operations.",
  ...
  },
  {
    ...
    per: "/mo",
    summary:
      "For pharmacy groups, hospital dispensaries, and multi-site operators.",
...
export function planChangeBlockers(
  input: { activeBranches: number; activeStaff: number },
  ...
  if (input.activeBranches > branchLimit)
    blockers.push(
      `${plan.name} allows ${branchLimit} active branch${branchLimit === 1 ? "" : "es"}.`,
    );
  if (input.activeStaff > staffLimit)
    blockers.push(`${plan.name} allows up to ${staffLimit} active staff.`);
  return blockers;
}
