...
function discountLimitPercent(db: Database, user: User) {
  ...
  const policy = pricingPolicy(db.settings);
  if (db.branches.some((branch) => canManageBranch(db, user, branch.id)))
    return policy.managerDiscountLimitPercent;
  if (user.role === "pharmacist" || user.role === "inventory")
    return policy.managerDiscountLimitPercent;
  ...
  return {
    ...
    settings: {
      ...
      trialEndsAt: new Date(
        Date.now() + trialPolicy.durationDays * 24 * 60 * 60 * 1000,
      ).toISOString(),
    },
  };
}
...
const builtInMedicineLabelRules: MedicineLabelRule[] = [
  ...
  },
  {
    ...
    followUpMessage:
      "Please complete the antibiotic course exactly as directed, even if you feel better. Contact the pharmacy if you develop rash, severe diarrhea, swelling, or breathing difficulty.",
    enabled: true,
  },
  {
    id: "built-in-aprovel",
  ...
  },
  {
    ...
    followUpMessage:
      "Remember to take your diabetes medicine with meals unless your prescriber advised differently. Keep monitoring your blood sugar and contact the pharmacy if you feel unwell.",
    enabled: true,
  },
  {
    id: "built-in-bp",
    ...
    followUpMessage:
      "Please take your blood pressure medicine at the same time daily and keep monitoring your readings. Contact the pharmacy if you notice severe dizziness or swelling.",
    enabled: true,
  },
  {
...
function defaultLabelInstruction(
  ...
  return combinedInstruction(
    ...
    `Take ${medicineName} exactly as prescribed. Contact the pharmacy if you notice unusual side effects.`,
  );
}
...
function medicineFollowUpInstruction(medicine: Medicine) {
  ...
  return (
    followUp ||
    `Take ${medicine.brandName} exactly as prescribed and contact the pharmacy if you notice unusual side effects.`
  );
}

function buildPurchaseFollowUpMessage(
  db: Database,
...
function findReceivableItem(
  ...
  | { itemType: "medicine" | "product"; itemId: string; label: string }
  | undefined {
  ...
  const productMatches = db.products
    ...
    .map((product) => ({
      ...
      itemId: product.id,
      label: product.name,
      exact: [product.sku, ...product.barcodes].some(
        (value) => value.toLowerCase() === needle,
      ),
...
function receivedRequisitionNotification(
  ...
  const summary = request.items
    ...
    .join(", ");
  const extraCount = request.items.filter((item) => (item.receivedQuantity ?? 0) > 0).length - 3;
  return {
...
function App() {
  ...
  const forgetBrowserUser = useCallback(
    (userId?: string | null) => {
      ...
      if (!userId) return;
      const workspaceKey =
        db.settings.companySlug || db.settings.companyCode || "workspace";
  ...
  async function createFirstAdmin(input: SetupInput) {
    ...
    try {
      ...
      if (
        result.db.settings.companySlug &&
        getWorkspaceSlugFromLocation() !== result.db.settings.companySlug
      ) {
  ...
  if (!currentUser) {
    ...
    return (
      <AuthScreen
        hasUsers={
          authIntent === "setup" && !isWorkspaceRoute ? false : hasUsers
        }
        tenantExists={
          authIntent === "setup" && !isWorkspaceRoute ? false : tenantExists
        }
        companySlug={
          authIntent === "setup" && !isWorkspaceRoute ? "" : companySlug
        }
...
function ReceiveStock({
  ...
  function buildReviewLines() {
    if (!selectedSupplierId || !hasReceivableItems || !activeBranch.id) {
      flash(
        "Add at least one Pharmacy or Mart item, supplier, and branch before receiving stock",
      );
      return [];
    }
      ...
      <form className="form-grid" onSubmit={submit}>
        ...
        <div className="line-editor full">
          {lines.map((line, index) => {
            ...
            const productOptions = db.products.filter(
                ...
                <div className="form-grid">
                  <label>
                    ...
                    >
                      ...
                      <option value="product">Mart</option>
                    </select>
                  </label>
                  <label className="full">
                    Item
                  ...
                  <label>
                    Batch/Lot number
                    <input
                      required={line.itemType === "medicine"}
...
function POSView({
  ...
  const allSaleOptions: SaleOption[] = useMemo(
    () => [
      ...db.medicines
        .filter(
          (medicine) =>
            medicine.active && (stockByMedicine.get(medicine.id) ?? 0) > 0,
        )
        .map((medicine) => {
          const batchPrice =
            stockRows
  ...
  const filteredSalesDiscount = filteredSales.reduce(
  ...
  const paymentTotals = filteredSales.reduce<Record<string, number>>(
    (totals, sale) => {
      totals[sale.paymentMethod] =
        (totals[sale.paymentMethod] ?? 0) + (sale.total ?? sale.subtotal);
      return totals;
    },
    {},
  );
        ...
        return current.map((item) =>
          item.rowId === existing.rowId
            ? {
                ...
                quantity: Math.min(
                  item.quantity + 1,
                  option.available || item.quantity + 1,
                ),
              }
            : item,
        );
  ...
  async function saveDraft() {
    ...
    const saved = await executeAction(
      "savePosDraft",
      salePayload(),
      "POS cart saved as a 10-minute draft",
    );
  ...
  function printSalesHistory() {
    ...
    const paymentSummary = Object.entries(paymentTotals)
      .map(
        ([method, amount]) =>
          `<li>${escapeHtml(method)}: ${money.format(amount)}</li>`,
      )
      .join("");
    const win = window.open("", "_blank", "width=900,height=700");
    if (!win) return;
      ...
      <div className="pos-window">
        <div className="pos-appbar">
          ...
          <div className="pos-appbar-right">
            <span className="pos-shift-pill">
              <span /> Shift open · {currentUser.name}
            </span>
        ...
        {!canSell && (
          <div className="form-error pos-access-error">
            You need cashier, pharmacist, branch manager, or admin access in{" "}
            {activeBranch.name} to use POS.
          </div>
        )}
        ...
        <form className="pos-terminal-grid" onSubmit={submit}>
          <section className="pos-catalog-panel">
            ...
            <div className="pos-quick-heading">
              <div>
                <strong>
                  {query.trim() ? "Search results" : "Frequently sold items"}
                </strong>
                <span>
                  {query.trim()
                    ? "Matching stocked Pharmacy and Mart items"
                    : "Based on real sales history and current stock availability"}
                </span>
              </div>
            </div>
            ...
            <div className="pos-catalog-grid">
              {visibleSaleOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    disabled={!canSell || option.unitPrice <= 0}
                  >
                    <div>
              ...
              {!visibleSaleOptions.length && (
                <div className="empty-state">
                  {query.trim()
                    ? "No stocked medicines or retail products match this search."
                    : "No frequently sold stocked items yet. Use search or scan to sell the first items."}
                </div>
              )}
            </div>
          ...
          <aside className="pos-sale-cart">
            <div className="pos-cart-header">
              <div>
                ...
                <p>
                  ...
                  ready / FEFO batch auto-selected
                </p>
              </div>
              <button
            ...
            <div className="pos-cart-lines">
              ...
              {!cartRows.length && (
                <div className="empty-state">
                  Add medicines or products to begin a sale.
                </div>
              )}
            </div>
            ...
            <div className="pos-customer-panel">
              ...
              <label>
                ...
                <small
                  className={
                    discountTooHigh
                      ? "discount-limit-note danger"
                      : "discount-limit-note"
                  }
                >
                  Limit: {maxDiscountPercent}% /{" "}
                  {money.format(maxDiscountAmount)}
                </small>
              </label>
              <label>
                <span>
                  <Wallet size={15} /> Cash paid
                </span>
                <input
                  type="number"
                  min="0"
                  value={numberInputValue(cashPaid)}
                  onChange={(event) => setCashPaid(Number(event.target.value))}
              ...
              <label>
                ...
                <input
                  ...
                  placeholder="Add optional sale note"
                  disabled={!canSell}
                />
              </label>
              <label className="full">
                <span>
                  <MessageSquare size={15} /> Purchase follow-up
                </span>
                <textarea
                  ...
                  placeholder={
                    ...
                    "One follow-up message for this purchase"
                  }
                  disabled={!canSell}
                  rows={3}
                />
            ...
            <div className="pos-total-panel">
              ...
              <div>
                <span>Discount</span>
                <strong>-{money.format(safeDiscount)}</strong>
              </div>
              <div>
                <span>VAT (incl.)</span>
                <strong>{money.format(vatIncluded)}</strong>
              </div>
              <hr />
              <section>
                <div>
            ...
            <div className="pos-action-row">
              <button
                type="button"
                onClick={saveDraft}
                disabled={!canSell || !cartRows.length || subtotal <= 0}
              >
                <Save size={16} />
                Draft
              </button>
              ...
              <button
                className="complete"
                type="submit"
                disabled={!canCompleteSale || !cartRows.length || total <= 0}
                title={
                  canCompleteSale
                    ? "Complete sale"
        ...
        >
          ...
          >
            ...
            <div className="pos-sales-summary">
              ...
              <div>
                <span>Discount</span>
                <strong>{money.format(filteredSalesDiscount)}</strong>
              </div>
              <div>
                <span>Total</span>
            ...
            <div className="pos-payment-summary">
              {Object.entries(paymentTotals).map(([method, amount]) => (
                <span key={method}>
                  {method}: <strong>{money.format(amount)}</strong>
                </span>
              ))}
              {!Object.keys(paymentTotals).length && (
                ...
                <article className="sale-history-item" key={sale.id}>
                  <div>
                    ...
                    <span>
                      ...
                      Cashier {getUserName(db, sale.cashierUserId)}
                    </span>
                  </div>
                  <span>
                  ...
                  {sale.discount > 0 && (
                    <small>Discount: {money.format(sale.discount)}</small>
                  )}
                </article>
              ))}
        ...
        >
          ...
          >
            ...
            <div className="pos-modal-list">
              ...
              {!branchDrafts.length && (
                <div className="empty-state">
                  No active POS drafts for this branch.
                </div>
              )}
            </div>
          </section>
...
function Reports({
  ...
  const movementSalesTotal =
    report === "movement"
      ? rows.reduce((sum, row) => sum + (Number(row["Sales Value"]) || 0), 0)
      : 0;
...
function SettingsView({
  ...
  const currentPlanId =
    db.settings.subscriptionPlanId ?? trialPolicy.includedPlan;
  ...
  const planBlockers =
    selectedPlanId === currentPlanId
      ? []
      : planChangeBlockers(activeUsage, selectedPlanId);
  const trialEnds = form.trialEndsAt ? formatDate(form.trialEndsAt) : "";
  const workspaceUrl = getWorkspaceUrl(db.settings);
  const assignedBranches = isSuperAdmin(db, currentUser)
    ? "All branches"
    : db.branches
        .filter((branch) => hasActiveBranchAssignment(currentUser, branch.id))
        .map((branch) => branch.name)
        .join(", ") || "No active branch assignment";
    ...
    <section className="content-section narrow">
      <div className="section-heading">
        <div>
          ...
          <p>Company account identity, workspace access, and operational thresholds.</p>
        </div>
      </div>
      <section className="user-profile-card">
        ...
        <div className="user-profile-grid">
          <div className="user-profile-meta">
            ...
            <small>{statusLabels[currentUser.status]}</small>
          </div>
          <div className="user-profile-meta">
            <span>Phone</span>
            <strong>{currentUser.phone || "Not recorded"}</strong>
            <small>Used for staff contact and security review.</small>
          </div>
          <div className="user-profile-meta">
            <span>Branch access</span>
            <strong>{assignedBranches}</strong>
            <small>{isSuperAdmin(db, currentUser) ? "Permanent admin access" : "Assigned by branch manager or admin"}</small>
          </div>
        </div>
      </section>
      <section className="workspace-access-card">
        <div>
          ...
          <p>Share these with approved staff so they can find the correct company workspace before signing in.</p>
        </div>
      ...
      <form className="form-grid" onSubmit={submit}>
        <div className="workspace-logo-editor full">
          ...
          <div>
            ...
            <span>
              This replaces the default capsule mark inside your company
              workspace.
            </span>
          </div>
          <label className="file-button">
        ...
        <div className="counseling-settings full">
          <div className="pricing-settings-head">
            <div>
              ...
              <p>
                Suggested chips appear in POS medication labels and print on
                medicine labels only. Receipts stay clean.
              </p>
            </div>
          </div>
          <div className="pricing-settings-grid">
            ...
            <label className="full">
              ...
              <textarea
                ...
                disabled={!canAdmin}
                rows={3}
                placeholder="Please contact the pharmacy if symptoms persist or side effects occur."
              />
            </label>
          </div>
        ...
        <div className="subscription-settings full">
          <div>
            ...
            {trialEnds && (
              <small>
                Trial data is preserved. Current trial ends {trialEnds}.
              </small>
            )}
          </div>
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setForm({
                  ...form,
                  subscriptionPlanId: event.target.value as SubscriptionPlanId,
                })
              }
              disabled={!canAdmin}
            >
          ...
          <div className="subscription-plan-boundary">
            ...
            <span>
              {activeUsage.activeBranches} active branch
              {activeUsage.activeBranches === 1 ? "" : "es"} /{" "}
              {activeUsage.activeStaff} active staff
            </span>
            <ul>
          ...
          {planBlockers.length > 0 ? (
            <div className="form-error">
              ...
              <span>
                Archive, export, or deactivate extra usage before moving to this
                lower plan. No historical data is deleted.
              </span>
            </div>
          ) : (
            <div className="empty-state">
              Plan changes keep stock, sales, patient, branch, and audit history
              in the workspace.
            </div>
          )}
        </div>
        <div className="form-actions full">
          <button
            className="primary-button"
            type="submit"
            disabled={!canAdmin || planBlockers.length > 0}
          >
            <Settings size={17} />
            Save settings
          </button>
